...
class VideoProcessor:
    ...
    def extract_keyframes(self, frames_per_minute: int = 10, duration: Optional[float] = None, max_frames: Optional[int] = None) -> List[Frame]:
        ...
        # Determine dynamic max_frames if not explicitly provided
        if max_frames is None:
            if video_duration <= 15:
                ...
                max_frames_cap = 15
            logger.info(f"Dynamically determined max_frames cap: {max_frames_cap} for video duration: {video_duration:.1f}s")
        else:
